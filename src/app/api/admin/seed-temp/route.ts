...
export async function GET() {
  try {
    ...
    // Relation-dependent tables first
    ...
    console.log("🏙️ Seeding Infrastructure Projects (10 projects)...");
    const seededInfra = [];
    ...
    console.log("⚖️ Seeding Legal Risks...");
    for (const r of legalRisksData) {
      await prisma.legalRisk.create({ data: r });
    }

    console.log("📊 Seeding Market Pulse snapshot...");
    ...
    console.log("📈 Seeding appreciation history, demand trends, and approvals for each corridor...");
      ...
      for (const y of years) {
        ...
        await prisma.appreciationHistory.create({
          data: {
            corridor: c.slug,
            year: y,
            pricePerSqFt: parseFloat((price / 9).toFixed(1)),
            pricePerSqYd: price,
    ...
    console.log("🧮 Recomputing all corridor scores...");
    const scores = await computeAllCorridorScores();
